...
```console
cargo build --locked --release -p a14
cargo test --locked -p a14 -- --test-threads=2
```

The executable is `target/release/a14.exe` on Windows or `target/release/a14`
on Linux. `build.cmd -App a14` also copies it to `dist/a14/`.
From beside the executable:

```powershell
.\a14.exe keygen secret.key
.\a14.exe encrypt input.dat encrypted.enc --key secret.key
.\a14.exe decrypt encrypted.enc restored.dat --key secret.key
```
...
Algorithm ID: 5; credential mode: 2.
