...
```console
cargo build --locked --release -p a11
cargo test --locked -p a11 -- --test-threads=2
```

The executable is `target/release/a11.exe` on Windows or `target/release/a11`
on Linux. `build.cmd -App a11` also copies it to `dist/a11/`.
From beside the executable:

```powershell
.\a11.exe keygen secret.key
.\a11.exe encrypt input.dat encrypted.enc --key secret.key
.\a11.exe decrypt encrypted.enc restored.dat --key secret.key
```
...
Algorithm ID: 4; credential mode: 2.
