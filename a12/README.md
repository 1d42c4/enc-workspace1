...
```console
cargo build --locked --release -p a12
cargo test --locked -p a12 -- --test-threads=2
```

The executable is `target/release/a12.exe` on Windows or `target/release/a12`
on Linux. `build.cmd -App a12` also copies it to `dist/a12/`.
From beside the executable:

```powershell
.\a12.exe encrypt input.dat encrypted.enc
.\a12.exe decrypt encrypted.enc restored.dat
```
...
Algorithm ID: 4; credential mode: 3.
