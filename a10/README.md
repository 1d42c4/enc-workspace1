...
```console
cargo build --locked --release -p a10
cargo test --locked -p a10 -- --test-threads=2
```

The executable is `target/release/a10.exe` on Windows or `target/release/a10`
on Linux. `build.cmd -App a10` also copies it to `dist/a10/`.
From beside the executable:

```powershell
.\a10.exe encrypt input.dat encrypted.enc
.\a10.exe decrypt encrypted.enc restored.dat
```
...
Algorithm ID: 4; credential mode: 1.
