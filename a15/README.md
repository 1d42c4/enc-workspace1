# a15: AES-128-GCM-SIV / embedded-key
...
```console
cargo build --locked --release -p a15
cargo test --locked -p a15 -- --test-threads=2
```

The executable is `target/release/a15.exe` on Windows or `target/release/a15`
on Linux. `build.cmd -App a15` also copies it to `dist/a15/`.
From beside the executable:

```powershell
.\a15.exe encrypt input.dat encrypted.enc
.\a15.exe decrypt encrypted.enc restored.dat
```
...
Algorithm ID: 5; credential mode: 3.
