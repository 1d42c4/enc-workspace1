...
```console
cargo build --locked --release -p a8
cargo test --locked -p a8 -- --test-threads=2
```

The executable is `target/release/a8.exe` on Windows or `target/release/a8`
on Linux. `build.cmd -App a8` also copies it to `dist/a8/`.
From beside the executable:

```powershell
.\a8.exe keygen secret.key
.\a8.exe encrypt input.dat encrypted.enc --key secret.key
.\a8.exe decrypt encrypted.enc restored.dat --key secret.key
```
...
Algorithm ID: 3; credential mode: 2.
