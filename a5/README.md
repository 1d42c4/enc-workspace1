# a5: XChaCha20-Poly1305 / key-file
...
```console
cargo build --locked --release -p a5
cargo test --locked -p a5 -- --test-threads=2
```

The executable is `target/release/a5.exe` on Windows or `target/release/a5`
on Linux. `build.cmd -App a5` also copies it to `dist/a5/`.
From beside the executable:

```powershell
.\a5.exe keygen secret.key
.\a5.exe encrypt input.dat encrypted.enc --key secret.key
.\a5.exe decrypt encrypted.enc restored.dat --key secret.key
```
...
Algorithm ID: 2; credential mode: 2.
