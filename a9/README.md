...
```console
cargo build --locked --release -p a9
cargo test --locked -p a9 -- --test-threads=2
```

The executable is `target/release/a9.exe` on Windows or `target/release/a9`
on Linux. `build.cmd -App a9` also copies it to `dist/a9/`.
From beside the executable:

```powershell
.\a9.exe encrypt input.dat encrypted.enc
.\a9.exe decrypt encrypted.enc restored.dat
```
...
Algorithm ID: 3; credential mode: 3.
