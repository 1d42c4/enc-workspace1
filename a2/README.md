...
```console
cargo build --locked --release -p a2
cargo test --locked -p a2 -- --test-threads=2
```

The executable is `target/release/a2.exe` on Windows or `target/release/a2`
on Linux. `build.cmd -App a2` also copies it to `dist/a2/`.
From beside the executable:

```powershell
.\a2.exe keygen secret.key
.\a2.exe encrypt input.dat encrypted.enc --key secret.key
.\a2.exe decrypt encrypted.enc restored.dat --key secret.key
```
...
Algorithm ID: 1; credential mode: 2.
