# a6: XChaCha20-Poly1305 / embedded-key
...
```console
cargo build --locked --release -p a6
cargo test --locked -p a6 -- --test-threads=2
```

The executable is `target/release/a6.exe` on Windows or `target/release/a6`
on Linux. `build.cmd -App a6` also copies it to `dist/a6/`.
From beside the executable:

```powershell
.\a6.exe encrypt input.dat encrypted.enc
.\a6.exe decrypt encrypted.enc restored.dat
```
...
Algorithm ID: 2; credential mode: 3.
