...
```console
cargo build --locked --release -p a1
cargo test --locked -p a1 -- --test-threads=2
```

The executable is `target/release/a1.exe` on Windows or `target/release/a1`
on Linux. `build.cmd -App a1` also copies it to `dist/a1/`.
From beside the executable:

```powershell
.\a1.exe encrypt input.dat encrypted.enc
.\a1.exe decrypt encrypted.enc restored.dat
```
...
Algorithm ID: 1; credential mode: 1.
