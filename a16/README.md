...
```console
cargo build --locked --release -p a16
cargo test --locked -p a16 -- --test-threads=2
```

The executable is `target/release/a16.exe` on Windows or `target/release/a16`
on Linux. `build.cmd -App a16` also copies it to `dist/a16/`.
From beside the executable:

```powershell
.\a16.exe encrypt input.dat encrypted.enc
.\a16.exe decrypt encrypted.enc restored.dat
```
...
Algorithm ID: 6; credential mode: 1.
