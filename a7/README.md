...
```console
cargo build --locked --release -p a7
cargo test --locked -p a7 -- --test-threads=2
```

The executable is `target/release/a7.exe` on Windows or `target/release/a7`
on Linux. `build.cmd -App a7` also copies it to `dist/a7/`.
From beside the executable:

```powershell
.\a7.exe encrypt input.dat encrypted.enc
.\a7.exe decrypt encrypted.enc restored.dat
```
...
Algorithm ID: 3; credential mode: 1.
